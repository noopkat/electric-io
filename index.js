...
var platform = process.env.PLATFORM || "default";
...
if (simulating === "true") {
  ...
  if (!connectionString) {
    console.error(
      new Error("oops, you're missing a CONNECTION_STRING entry in ./.env!")
    );
    process.exit();
  }
...
app.post("/api/device/:deviceId/method/:deviceMethod", function(req, res) {
  ...
  client.invokeDeviceMethod(req.params.deviceId, methodParams, function(
    err,
    result
  ) {
    if (err) {
