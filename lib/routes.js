  ...
  routes.post("/api/device/:deviceId/method/:deviceMethod", function(req, res) {
    ...
    hubService.callDeviceMethod(req.params.deviceId, methodParams, function(
      ...
      if (error) {
        ...
        debug(
          `Successful method call '${methodParams.methodName}' on ${req.params.deviceId}`
        );
  ...
  routes.post("/api/device/:deviceId/message/", function(req, res) {
    ...
    hubService.callDeviceMessage(
      ...
      function(error, result) {
        if (error) {
          console.error(`Failed to queue message: ${error.toString()}`);
          res.sendStatus(500);
        } else {
          debug(`Successfully queued message on ${req.params.deviceId}`);
          debug(JSON.stringify(result, null, 2));
          res.sendStatus(200);
        }
      }
